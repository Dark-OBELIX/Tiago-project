...
class ArucoDocker:
    def __init__(self):
        # On retire anonymous=True pour avoir un nom fixe "tiago_aruco_docking"
        rospy.init_node('tiago_aruco_docking', anonymous=False)

        # Service Server
        self.service = rospy.Service('start_aruco_docking', Trigger, self.handle_service)
        
        self.target_id = 0
        rospy.loginfo("Service 'start_aruco_docking' pret. En attente de requete...")

        # --- REGLAGES UTILISATEUR ---
        self.marker_size = 0.164
        self.target_dist = 0.58   # Distance finale souhaitée
        ...
        self.state = "IDLE" # Start in IDLE mode waiting for service call
    ...
    def handle_service(self, req):
        # On lit l'ID depuis un parametre global pour faciliter l'interface
        self.target_id = rospy.get_param('/aruco_to_dock', 0)
        
        rospy.loginfo(f"REQUETE RECUE. Demarrage docking ArUco ID: {self.target_id}")
        
        # 1. Lever la tete pour bien voir l'ArUco
        self.look_forward()
        rospy.sleep(2.0) # On attend que la tete soit en position
        ...
        # Boucle bloquante pour le service (attente de fin)
        rate = rospy.Rate(5)
        while not rospy.is_shutdown():
            if self.state == "DONE":
                self.state = "IDLE"
                rospy.loginfo("Service termine avec SUCCES.")
                return TriggerResponse(success=True, message="Docking successful")
            
            if self.state == "FAILED":
                self.state = "IDLE"
                rospy.logwarn("Service termine avec ECHEC.")
                return TriggerResponse(success=False, message="Docking failed (MoveBase or other)")
            
            rate.sleep()
            
        return TriggerResponse(success=False, message="Node shutdown")
    ...
    def done_cb(self, status, result):
        if status == 3: # SUCCEEDED
            ...
            self.state = "FAILED" # On signale la faute au service
    ...
    def process_visual_servoing(self, x_cam, z_cam):
        ...
        if abs(error_distance) < 0.01:
            ...
            self.stop_robot()
            self.state = "DONE"
            # On retire le shutdown pour garder le service actif
            return
