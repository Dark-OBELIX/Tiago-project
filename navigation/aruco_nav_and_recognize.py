...
class ArucoDocker:
    def __init__(self):
        ...
        self.state = "IDLE" # IDLE -> NAV_APPROACH -> VISUAL_SERVOING -> DONE
    ...
    def image_callback(self, data):
        ...
        if ids is not None and len(ids) > 0:
            # Pose Marker dans repere CAMERA
            rvec, tvec, _ = aruco.estimatePoseSingleMarkers(corners, self.marker_size, self.mtx, self.dist_coeff)
            
            # tvec[0][0] = X (Lateral: Droite/Gauche)
            # tvec[0][1] = Y (Vertical: Haut/Bas)
            # tvec[0][2] = Z (Profondeur: Distance)
            x_cam = tvec[0][0][0]
            z_cam = tvec[0][0][2]

            # Dessin Debug
            aruco.drawAxis(cv_image, self.mtx, self.dist_coeff, rvec[0], tvec[0], 0.05)
            cv2.putText(cv_image, f"DIST: {z_cam:.2f}m", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            # --- MACHINE A ETATS ---

            if self.state == "IDLE":
                # Si on voit un marker, on lance l'approche grossière
                rospy.loginfo("ArUco vu. Lancement move_base vers point d'approche...")
                
                tf_map_cam = self.get_transform("map", "xtion_rgb_optical_frame")
                if tf_map_cam:
                    self.start_coarse_approach(rvec[0], tvec[0], tf_map_cam)

            elif self.state == "VISUAL_SERVOING":
                # Ici, on ignore move_base, on pilote directement les roues
                self.process_visual_servoing(x_cam, z_cam)

        else:
            # Si on perd le marker pendant le servoing, on s'arrete par securité
